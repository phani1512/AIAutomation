...
function editSavedPrompt(index) {
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 600px;">
            <h3 style="color: var(--text-primary); margin-bottom: 20px;">✏️ Edit Saved Prompt</h3>
            ...
            <div style="display: flex; gap: 10px;">
                <button class="btn" onclick="updateSavedPrompt(${index})" style="flex: 1; background: var(--success);">
                    💾 Update
                </button>
...
function showAddPromptModal() {
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 600px;">
            <h3 style="color: var(--text-primary); margin-bottom: 20px;">➕ Save Prompt</h3>
            ...
            <div style="display: flex; gap: 10px;">
                <button class="btn" onclick="saveNewPrompt()" style="flex: 1; background: var(--success);">
                    💾 Save
                </button>
                <button class="btn" onclick="this.closest('.modal-overlay').remove()" style="flex: 1;">
                    Cancel
...
function savePromptToAccount() {
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 500px;">
            <h3 style="margin-bottom: 20px;">💾 Save Prompt</h3>
            ...
            <div style="display: flex; gap: 10px; margin-top: 20px;">
                <button class="btn" onclick="window.confirmSavePrompt('${prompt.replace(/'/g, "\\'")}');" style="flex: 1;">
                    💾 Save
                </button>
...
function showSavedPrompts() {
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 700px; max-height: 80vh; overflow: auto;">
            <h3 style="margin-bottom: 20px;">📚 My Saved Prompts (${prompts.length})</h3>
            ${prompts.length > 0 ? `
                        ...
                        <div style="padding: 15px; margin-bottom: 10px; background: var(--bg-secondary); border-radius: 8px; border-left: 4px solid var(--primary);">
                            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                                ...
                                <div style="display: flex; gap: 8px;">
                                    <button onclick="window.loadSavedPromptToInput(${index}); document.querySelector('.modal-overlay').remove();" style="background: var(--primary); color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer;">Use</button>
                                    <button onclick="window.deleteSavedAccountPrompt(${index}); document.querySelector('.modal-overlay').remove(); window.showSavedPrompts();" style="background: var(--error); color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer;">Delete</button>
                                </div>
                            </div>
            ...
            <button class="btn" onclick="this.closest('.modal-overlay').remove()" style="margin-top: 20px; width: 100%;">Close</button>
        </div>
    `;
    document.body.appendChild(modal);
}
